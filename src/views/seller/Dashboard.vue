...
<template>
    <div class="cont" v-if="state.isVerified == 'Approved'">
        <h1 class="text-center">DASHBOARD</h1>

        <Modal v-if="modal.categorySuggestion" @close="modal.categorySuggestion = false">
            <form class=" form category" @submit.prevent="requestNewCategory">
                <h2>Request For New Catgeory</h2>
                <input class="" type="text" v-model="form.category" placeholder="Enter Category Name" required>
        ...
        <div class="text-center">
            <button class="btn new_category" @click="modal.categorySuggestion = true">Want To Add New Category</button>
        </div>
    ...
    <div class="pending" v-else-if="state.isVerified == 'Pending'">
        <i class="fa-solid fa-hourglass-end" /> Your Seller Account Verification is Pending
        <br>
        Admin can approve your account in the next 24 hours.
    </div>

    <div v-else class="notVerified">
        Your Account is Rejected
        <br>
        Don't Worry You can Re Apply <br>
        <button class="btn" @click="approval">Apply For Approval</button>
    </div>
</template>
...
function requestNewCategory() {
    ...
    axios.post(API + '/category/request', { name: form.category }, { headers })
        .then(res => {

            modal.categorySuggestion = false
            form.category = ''
...
function approval() {
    axios.get(API + '/seller/apply', {headers}).
        then((res) => {
...
onMounted(() => {
    ...
    axios.get(API + '/seller/profile', { headers })
        .then(res => {
            state.isVerified = res.data.data.isVerified
